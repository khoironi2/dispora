...
class Evaluasi extends CI_Controller
{
    ...
    public function index()
    {
        $data = [
            ...
            'getall' => $this->Evaluasi_kepemudaan_model->getAll(),
        ];
        $this->load->view('template/operator/header', $data);
        $this->load->view('template/operator/sidebar');
        $this->load->view('operator/publikasi/berjalan/evaluasi', $data);
        $this->load->view('template/operator/footer', $data);
    }

    public function setuju($id)
    {
        $idpaskib = $this->db->get_where('tbl_pendaftar_evaluasi_kepemudaan', ['id_pendaftar_evaluasi_kepemudaan' => $id])->row_array();
        $status_lulus = "";
        ...
        $data = array(
            'id_pendaftar_evaluasi_kepemudaan'         => $id,
            'status_lulus'     => $status_lulus
        );

        $insert =  $this->Evaluasi_kepemudaan_model->update($id, $data);
        if ($insert) {
            // echo "success";
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Sukses, Data berhasil di perbarui !</div>');
            redirect('operator/publikasi/berjalan/evaluasi');
        } else {
