                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="symbol symbol-60 symbol-2by3 flex-shrink-0">
                                        <img class="symbol-label" src="<?= base_url('assets/img/kreatifitas/' . $key['file_foto_kreativitas']) ?>" />
                                    </div>
                                </td>
                                <td><?= $key['nama_program_kreativitas'] ?></td>
                                <td><?= $key['capaian_akhir'] ?></td>
                                <td><?= $key['status_terlaksana'] ?></td>
                                <td><?= $key['periode'] ?></td>
                                <td>
                                    <a data-toggle="modal" data-target="#edit<?= $key['id_kreatifitas_lembaga_pemuda'] ?>">
                                        <span style="font-size: 3em; color: red;">
                                            <i class="far fa-edit"></i>
                                        </span>
                                    </a>
                                    <a data-toggle="modal" data-target="#delete<?= $key['id_kreatifitas_lembaga_pemuda'] ?>">
                                        <span style="font-size: 3em; color: red;">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="login-form login-signup">
                    <!--begin::Form-->
                    <form action="<?= base_url('operator/kreatifitas_kelembagaan_pemuda/save') ?>" method="POST" class="form" enctype="multipart/form-data">
                        <!--begin::Title-->
                        <!--end::Title-->
                        <!--begin::Form group-->
                        <div class="form-group">
                            <label for="exampleSelectl">Nama</label>
                            <input class="form-control form-control-solid h-auto p-6 rounded-lg font-size-h6" type="text" placeholder="nama program" name="nama_program_kreativitas" id="nama_program_kreativitas" autocomplete="off" />
                        </div>
                        <div class="row">
                            <div class="col">
                                <div class="form-group">
                                    <label for="exampleSelectl">Capaian</label>
                                    <textarea name="capaian_akhir" id="capaian_akhir" class="form-control form-control-solid h-auto p-6 rounded-lg font-size-h6"></textarea>
                            ...
                            <div class="col">
                                <div class="image-input image-input-outline" id="kt_image_2">
                                    ...
                                    <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="" data-original-title="Change avatar">
                                        ...
                                        <input type="file" name="file_foto_kreativitas" accept=".png, .jpg, .jpeg" />
...
<?php foreach ($get as $key) : ?>
    <!-- Modal edit-->
    <div class="modal fade" id="edit<?= $key['id_kreatifitas_lembaga_pemuda'] ?>" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="login-form login-signup">
                        <!--begin::Form-->
                        <form action="<?= base_url('operator/kreatifitas_kelembagaan_pemuda/update') ?>" method="POST" class="form" enctype="multipart/form-data">
                            <!--begin::Title-->
                            <!--end::Title-->
                            <!--begin::Form group-->
                            <div class="form-group">
                                <label for="exampleSelectl">Nama</label>
                                <input class="form-control form-control-solid h-auto p-6 rounded-lg font-size-h6" type="text" placeholder="nama program" value="<?= $key['nama_program_kreativitas'] ?>" name="nama_program_kreativitas" id="nama_program_kreativitas" autocomplete="off" />
                                <input class="form-control form-control-solid h-auto p-6 rounded-lg font-size-h6" hidden type="text" placeholder="nama program" value="<?= $key['id_kreatifitas_lembaga_pemuda'] ?>" name="id_kreatifitas_lembaga_pemuda" id="id_kreatifitas_lembaga_pemuda" autocomplete="off" />
                            </div>
                            <div class="row">
                                <div class="col">
                                    <div class="form-group">
                                        <label for="exampleSelectl">Capaian</label>
                                        <textarea name="capaian_akhir" id="capaian_akhir" class="form-control form-control-solid h-auto p-6 rounded-lg font-size-h6"><?= $key['capaian_akhir'] ?></textarea>

                                    </div>
                                </div>
                                <div class="col">
                                    <div class="image-input image-input-outline" id="kt_image_3">
                                        <div class="image-input-wrapper" style="background-image: url(<?= base_url('assets/img/kreatifitas/' . $key['file_foto_kreativitas']) ?>)"></div>
                                        <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="" data-original-title="Change avatar">
                                            <i class="fa fa-pen icon-sm text-muted"></i>
                                            <input type="file" name="file_foto_kreativitas" accept=".png, .jpg, .jpeg" />
                                            <input type="hidden" name="profile_avatar_remove" />
                                        ...
                                        <div class="input-group input-group-solid date" id="kt_datetimepicker_3" data-target-input="nearest">
                                            <select name="status_terlaksana" class="form-control form-control-solid h-auto p-6 rounded-lg font-size-h6" id="status_terlaksana">
                                                <?php if ($key['status_terlaksana'] == 1) { ?>
                                                    <option value="<?= $key['status_terlaksana'] ?>">On Going</option>
                                                <?php } elseif ($key['status_terlaksana'] == 2) { ?>
                                                    <option value="<?= $key['status_terlaksana'] ?>">Terlaksana</option>
                                                <?php } ?>
                                                <option value="1">On Going</option>
                                                <option value="2">Terlaksana</option>
...
<?php foreach ($get as $key) : ?>
    <!-- Modal delete-->
    <div class="modal fade" id="delete<?= $key['id_kreatifitas_lembaga_pemuda'] ?>" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <form action="<?= base_url('operator/kreatifitas_kelembagaan_pemuda/delete/' . $key['id_kreatifitas_lembaga_pemuda']) ?>" method="POST">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body text-danger">
                        <h5> Yakin ingin menghapus <span class="text-dark text-bold"><?= $key['nama_program_kreativitas'] ?></span> </h5>
                    </div>
